...
# 用法：cd HoneyPot && bash honeypot/scripts/setup.sh
#      或  cd HoneyPot/honeypot && bash scripts/setup.sh
set -e
...
if [ -z "$SKIP_MODELS" ]; then
    ...
    REPORT_MODEL=""
    if [ -f ".env" ]; then
        TERMINAL_MODEL=$(grep "^OLLAMA_MODEL=" .env | cut -d= -f2 | tr -d '[:space:]')
        REPORT_MODEL=$(grep "^OLLAMA_REPORT_MODEL=" .env | cut -d= -f2 | tr -d '[:space:]')
    fi
    ...
    pull_if_missing() {
        ...
        if ollama list 2>/dev/null | grep -qF "$model"; then
            ...
            echo "  ↓ 下載 $model（約 $([ "${model##*:}" = "8b-instruct-q8_0" ] && echo "8.5GB" || echo "9GB")，可能需要數分鐘）..."
            ollama pull "$model"
            echo "  ✓ $model 下載完成"
        fi
    }
